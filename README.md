...
2. **Lattice**: A Lattice Network (unlike LSTM and GRU) supports distinct outputs along depth and time. Hence we implemented `class Lattice` (in `src/model.py`) which supports recurrent units with 2 different outputs. Batches with multiple length sequences is allowed if they are converted to `torch.nn.utils.rnn.PackedSequence`. In addition, `Lattice` also supports multiple layers within an RNN cell (`class LRUxCell` and `class HIGHWAYxCell` in `src/model.py`)
